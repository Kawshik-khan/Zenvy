const { invalidateConversationUnread, invalidateStudyMetrics } = require("./cache-runtime");
...
async function createConversationMessage(prisma, params) {
  const conversation = await assertCanAccessConversation(prisma, params.senderId, params.conversationId);
...
function registerConversationHandlers({ io, socket, prisma, userId }) {
  ...
  socket.on("conversation:join", async (data) => {
    ...
    try {
      const conversation = await assertCanAccessConversation(prisma, userId, data.conversationId);
